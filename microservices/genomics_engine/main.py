...
DISEASE_MODELS = {
    "alzheimers": {
        "id": "p4", "disease": "Late-Onset Alzheimer's", "base_risk": 5, "max_risk": 97, "base_conf": 85,
        "genes": "APOE, CLU, PICALM", "transition_risk_per": 4.5, "transversion_risk_per": 1.2,
        "specific_subs": [("C","T",12.0),("G","A",8.0),("A","G",3.5)],
        "high_threshold": 60, "med_threshold": 35,
        ...
        "insight_high": "High-penetrance C>T transition signature detected. Associated with early APOE e4 allele expression and accelerated neurodegeneration risk.",
    },
    "brca": {
        "id": "p3", "disease": "Breast Cancer (BRCA)", "base_risk": 2, "max_risk": 96, "base_conf": 95,
        "genes": "BRCA1, BRCA2, PALB2", "transition_risk_per": 1.0, "transversion_risk_per": 6.5,
        "specific_subs": [("A","T",15.0),("G","C",10.0),("T","A",9.0),("C","A",7.0)],
        "high_threshold": 50, "med_threshold": 20,
        "insight_low": "No pathogenic BRCA1/BRCA2 variant signature identified. Standard screening schedule recommended.",
        "insight_med": "Moderate transversion burden found. Enhanced imaging screening advised.",
        "insight_high": "High-confidence pathogenic A>T transversion detected. Genetic counselling strongly recommended.",
    },
    "diabetes": {
        "id": "p1", "disease": "Type 2 Diabetes", "base_risk": 10, "max_risk": 92, "base_conf": 88,
        "genes": "TCF7L2, KCNQ1, SLC30A8", "transition_risk_per": 2.0, "transversion_risk_per": 3.5,
        "specific_subs": [("T","A",12.0),("G","T",8.0),("C","G",5.5)],
        "high_threshold": 60, "med_threshold": 35,
        "insight_low": "TCF7L2 and KCNQ1 loci show no elevated variant burden. Baseline metabolic risk profile.",
        "insight_med": "Elevated transversion burden near TCF7L2 locus. Lifestyle intervention recommended.",
    ...
    "cad": {
        "id": "p2", "disease": "Coronary Artery Disease", "base_risk": 15, "max_risk": 93, "base_conf": 92,
        "genes": "APOB, LDLR, PCSK9", "transition_risk_per": 2.5, "transversion_risk_per": 3.0,
        "specific_subs": [("G","C",11.0),("A","C",7.0),("T","G",6.0),("C","T",4.0)],
        "high_threshold": 60, "med_threshold": 35,
        "insight_low": "APOB and LDLR loci show standard baseline profile.",
        "insight_med": "Moderate G>C transversion burden detected. Lipid panel monitoring advised.",
        "insight_high": "Strong pathogenic signal in LDLR/PCSK9 binding domain. Cardiology referral recommended.",
    },
    "hiv": {
        "id": "p5", "disease": "HIV Susceptibility", "base_risk": 3, "max_risk": 88, "base_conf": 82,
        "genes": "CCR5, HLA-B, TRIM5", "transition_risk_per": 3.0, "transversion_risk_per": 2.0,
        "specific_subs": [("G","A",14.0),("C","T",10.0),("A","C",6.0),("T","C",5.0)],
        "high_threshold": 55, "med_threshold": 25,
        "insight_low": "No high-risk CCR5 or HLA-B variants detected. Standard prevention guidelines apply.",
        "insight_med": "Partial CCR5 promoter variant burden detected. PrEP consultation recommended.",
        "insight_high": "High-penetrance G>A transition signature in CCR5/HLA-B loci. Immediate immunology referral advised.",
    },
    "lung_cancer": {
        "id": "p6", "disease": "Lung Cancer", "base_risk": 4, "max_risk": 95, "base_conf": 90,
        "genes": "EGFR, KRAS, TP53, STK11", "transition_risk_per": 2.8, "transversion_risk_per": 5.5,
        "specific_subs": [("C","A",16.0),("G","T",14.0),("C","T",9.0),("A","T",7.0)],
        "high_threshold": 55, "med_threshold": 25,
        "insight_low": "No pathogenic EGFR, KRAS, or TP53 variant signatures identified.",
        "insight_med": "Moderate C>A transversion burden consistent with early carcinogen exposure signature.",
        "insight_high": "High-confidence KRAS G12C / TP53 loss-of-function signature detected. Immediate oncology referral indicated.",
    },
}


def _is_transition_pred(ref: str, query: str) -> bool:
    purines = {"A", "G"}
    pyrimidines = {"C", "T"}
    return (ref in purines and query in purines) or (ref in pyrimidines and query in pyrimidines)


def compute_risk(model: dict, mutations: list) -> tuple:
    ...
    for (ref, query), count in sub_counts.items():
        ...
        if any(ref == r and query == q for r, q, _ in model["specific_subs"]):
            continue
        per_mut = model["transition_risk_per"] if _is_transition_pred(ref, query) else model["transversion_risk_per"]
...
def build_prediction(model: dict, risk: float, confidence: float, total_mutations: int) -> dict:
    if risk >= model["high_threshold"]:
        severity, trend, insight = "high", "increasing", model["insight_high"]
    elif risk >= model["med_threshold"]:
        severity, trend, insight = "medium", "increasing", model["insight_med"]
    else:
        severity, trend, insight = "low", "stable", model["insight_low"]
    insight += f" ({total_mutations} variants analysed.)"
    return {"id": model["id"], "disease": model["disease"], "risk": int(risk),
            "confidence": int(confidence), "trend": trend, "severity": severity,
            "genes": model["genes"], "insight": insight}
